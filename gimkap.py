...
# Maybe better with this: http://docs.python.org/library/imghdr.html
...
if __name__ == "__main__":
    ...
    #######################################################################
    tiles_dir = os.path.expanduser("~/Maps/googlesat") # <----- Update this to your prefered location for storing GE tiles
    #######################################################################
    output_dir = "./kap/"
    ...
    for ty in range(tminy, tmaxy+1):
        for tx in range(tminx, tmaxx+1):
            ...
            if os.path.isfile(tilefilename):
                ...
                gmapsurl = "https://khms0.google.com/kh/v=113&src=app"
    ...
    try:
        ...
        if not os.path.exists(os.path.join(compositions_dir,filename)):
            os.makedirs(os.path.join(compositions_dir,filename))
        ...
        for k,v in res.items():
            ...
            convert += " -append %s" % os.path.join(os.path.join(compositions_dir,filename),"%s.jpg" % k)
            os.system( convert )

        print "Stitching job 2"
        os.system("convert %s/*.jpg +append %s.jpg" % (os.path.join(compositions_dir,filename),os.path.join(compositions_dir,filename)) )

        print
        print "Composition Map is generated in '%s.jpg'" % os.path.join(compositions_dir,filename)
        print "Creating kap file..."

        create_kap = "imgkap %s.jpg %f %f %f %f %s.kap" % (os.path.join(compositions_dir,filename), latmax,lng,lat,lngmax, os.path.join(output_dir,filename)) 
        print create_kap
